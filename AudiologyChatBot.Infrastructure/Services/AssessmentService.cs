...
namespace AudiologyChatBot.Infrastructure.Services
{
    public class AssessmentService : IAssessmentService
    {
        ...
        public async Task<DecisionNode?> GetQuestionAsync(string nodeId)
        {
            ...
            if (node != null && string.IsNullOrEmpty(node.Question) && !string.IsNullOrEmpty(node.Next))
            {
                return await GetQuestionAsync(node.Next);
            }
        ...
        public async Task<bool> SaveAnswerAsync(int patientId, SaveAnswerRequest request)
        {
            ...
            {
                _logger.LogError(ex, "Failed to save answer for patient {PatientId}", patientId);
                return false;
            }
        }
        ...
        // NEW: Method to get enriched assessment data for retrieval
